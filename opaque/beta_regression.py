        ...
        with pm.Model() as model:
            ...
            if self.coefficient_prior_type == 'normal':
                ...
                ValueError('coefficient_prior_type must be one of "normal" '
                           'or "laplace"')
            mu_est = pm.\
                Deterministic('mu_est',
                              pm.math.invlogit(int_mean +
                                               pm.math.dot(X_mean_obs,
                                                           beta_mean)))
            nu_est =pm.\
                Deterministic('nu_est',
                              pm.math.exp(int_disp +
                                           pm.math.dot(X_disp_obs, beta_disp)))
            y_est = pm.Beta('y_est', alpha=mu_est * nu_est,
                            beta = (1 - mu_est) * nu_est, observed=y_obs)
